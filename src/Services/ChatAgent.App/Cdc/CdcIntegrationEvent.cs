...
public abstract record CdcIntegrationEvent : IntegrationEvent
{
    ...
    /// replays are routine, and re-applying identical state would needlessly invalidate the
    /// row's embedding. When neither side carries metadata the event is applied, which keeps
    /// messages published before <c>add.fields</c> working.
    /// </remarks>
